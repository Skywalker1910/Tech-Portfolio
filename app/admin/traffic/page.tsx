...
type Report = {
  ...
  pages:PageMetric[];
  visits:Visit[];
  breakdowns:{ devices:Breakdown[]; operatingSystems:Breakdown[]; browsers:Breakdown[]; viewports:Breakdown[]; locations:Breakdown[]; regions:Breakdown[]; sources:Breakdown[]; events:Breakdown[] };
...
export default function TrafficPage() {
  ...
  return <div className="mx-auto max-w-7xl p-6 md:p-10">
    ...
    {loading ? <div className="flex items-center gap-2 py-20 text-sm text-[var(--muted)]"><Loader2 className="animate-spin" size={16}/> Loading analytics…</div> : error && !report ? <div className="rounded-2xl border border-amber-500/30 bg-amber-500/10 p-5 text-sm text-amber-600">{error}</div> : report && <>
      ...
      <section className="mt-5 rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-5"><div className="flex items-center justify-between"><h2 className="text-sm font-semibold">Daily reach</h2><span className="text-[10px] text-[var(--sub-muted)]">Mandatory visits with optional page context</span></div><div className="mt-6 flex h-44 items-end gap-1">{report.daily.map((entry)=><div key={entry.day} className="group relative flex h-full flex-1 items-end"><div className="min-h-1 w-full rounded-t bg-gradient-to-t from-orange-600 to-amber-400 transition group-hover:opacity-80" style={{height:`${Math.max(3,entry.mandatoryVisits/max*100)}%`}}/><span className="pointer-events-none absolute bottom-full left-1/2 z-10 mb-2 hidden -translate-x-1/2 whitespace-nowrap rounded bg-black px-2 py-1 text-[10px] text-white group-hover:block">{entry.day}: {entry.mandatoryVisitors} visitors · {entry.mandatoryVisits} visits · {entry.views} consented views</span></div>)}</div></section>
